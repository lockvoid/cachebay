...
describe("Fragments lifecycle (Svelte)", () => {
  describe("identify", () => {
  ...
  describe("createFragment (reactive)", () => {
    it("updates when fragment data changes", async () => {
      const { cache } = createTestClient();
      ...
      expect(f.data?.email).toBe("u1@example.com");
      ...
      expect(f.data?.email).toBe("u1+updated@example.com");
